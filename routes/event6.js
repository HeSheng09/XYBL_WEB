...
router.get('/:eventid',function (req,res,next) {
    ...
    if(req.session.role==="false"){
        ...
        console.log("是管理人员！");
        res.render('NTUevent6',{userid:req.session.userid,role:req.session.role,eventid:req.session.eventid});
    }
})

router.get('/mid/:eventid',function (req,res,next) {
    req.session.eventid=req.params.eventid;
    if(req.session.role==="false"){
        console.log("是学生");
        res.render('Midevent',{userid:req.session.userid,role:req.session.role,eventid:req.session.eventid});
    }
})
...
// router.get('/NTU',function (req,res,next) {
//     console.log("wolaile!!!!")
//     if(req.session.role==="false"){
//         console.log("是学生");
//         res.redirect("/error")
